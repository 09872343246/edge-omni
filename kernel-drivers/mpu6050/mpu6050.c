...
static inline int mpu6050_write_reg(struct mpu6050_data *data ,u8 reg,u8  val){
        ...
        if(ret)
                dev_err(&data->client->dev,"write reg 0x%02X = 0x%02X failed: %d\n" ,reg,val,ret);
        return ret;
}
...
static int mpu6050_probe(struct i2c_client *client){
	...
	ret = mpu6050_write_reg(data,MPU6050_REG_PWR_MGMT_1,MPU6050_VAL_WAKEUP);
	if(ret){
		dev_err(&client->dev,"wakeup failed: %d\n",ret);
		goto err_free_data;
	}

	ret = mpu6050_write_reg(data,MPU6050_REG_ACCEL_CONFIG,MPU6050_VAL_ACCEL_2G);
        if(ret)
                goto err_free_data;
...
MODULE_DESCRIPTION("MPU6050 Input driver for Orange Pi Zero");
